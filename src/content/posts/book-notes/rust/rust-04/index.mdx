...
> There is a difference between `string` type and `String` type. string is allocated on the stack, has a finite, known length while String is allocated on the heap, and is length is unknown to us at compile time.
...
Unless... We use references and Borrowing
...
_slices_ let you reference a contiguous sequence of elements in a collention rather than the whole collection. A slice is a kind of reference, so it does not have ownership.
